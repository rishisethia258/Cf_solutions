...
//    #endif
    ...
    vector<vector<bool>>dp1(n,vector<bool>(m));
    ...
    for(int i=n-1;i>=0;i--)
    {
        for(int j=m-1;j>=0;j--)
        {
            ...
            if(i==n-1 && s[i][j]=='.')dp[i][j]=dp[i][j+1];
            else if(j==m-1 && s[i][j]=='.')dp[i][j]=dp[i+1][j];
            else if( s[i][j]=='.')dp[i][j]=(dp[i+1][j] | dp[i][j+1]);
        }
    }
    for(int i=0;i<n;i++)
    {
        ...
        {
            if(i==0 && j==0)continue;
            else if(i==0 && s[i][j]=='.')dp1[i][j]=dp1[i][j-1];
            else if(j==0 && s[i][j]=='.')dp1[i][j]=dp1[i-1][j];
            else if(s[i][j]=='.')dp1[i][j]=(dp1[i-1][j] | dp1[i][j-1]);
            dp1[i][j]=(dp1[i][j] & dp[i][j]);
            if(i==0 && j==1 && dp1[i][j])c++;
            if(i==1 && j==0 && dp1[i][j])c++;
            if(i==n-1 && j==m-2)c1+=dp1[i][j];
            if(i==n-2 && j==m-1)c1+=dp1[i][j];
//            cout<<i<<" "<<j<<" "<<dp1[i][j]<<endl;
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
            if(j==0)dp2[i][j]=(dp1[i][j]^1);
            else dp2[i][j]=((dp1[i][j]^1)+dp2[i][j-1]);
        }
    }
    ll a1=0,a2=0;
    bool b1=0;
    for(int i=1;i<n-1;i++)
    {
        for(int j=0;j<m;j++)
        {
            ...
            a2=dp2[i-1][m-1]-dp2[i-1][j];
            if(a1==j && a2==m-j-1)b1=1;
//            cout<<a1<<" "<<a2<<endl;
        }
    }
    int res=min(c,c1);
